...
export const labelingSlice = createSlice({
  ...
  reducers: {
    ...
    set_int_x: (state, action: PayloadAction<number>) => {state.current_action.initial_x_cord = action.payload},
    set_int_y: (state, action: PayloadAction<number>) => {state.current_action.initial_y_cord = action.payload},
    set_end_x: (state, action: PayloadAction<number>) => {state.current_action.final_x_cord = action.payload},
    set_end_y: (state, action: PayloadAction<number>) => {state.current_action.final_y_cord = action.payload},
    set_are_we_labeling: (state, action: PayloadAction<boolean>) => {state.current_action.are_we_currently_labeling = action.payload},

  },
});

export const {set_int_x, set_int_y, set_end_x, set_end_y, set_are_we_labeling } = labelingSlice.actions;
...
export const select_Image_Dimensions = (state: RootState) => ({x: state.labeling.current_image_size.x, y: state.labeling.current_image_size.y})
export const select_Current_Bbox_State = (state: RootState) => (state.labeling.current_action);
